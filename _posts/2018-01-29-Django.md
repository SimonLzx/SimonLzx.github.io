...
1. Python用正则表达式匹配数字`(?P<article_id>[0-9]+)`。意思是：将正则表达式匹配到的数字以`article_id`为组名去匹配，所以这里的组名必须要和响应函数的参数名一致
...
3. 因为Django框架中每一个响应（函数）对应一个URL，所以新建一个页面必须在`views.py`中添加一个响应函数和`urls.py`中添加这个页面的url
